...
define ('VERSION', 'dee-2.8.11');																		// Version of this script 
//Load a config file if it exists. Otherwise, use the values below
if( file_exists(dirname(__FILE__) . '/timthumb-config.php'))	require_once('timthumb-config.php');
if(! defined('DEBUG_ON') )					define ('DEBUG_ON', false);								// Enable debug logging to web server error log (STDERR)
if(! defined('DEBUG_LEVEL') )				define ('DEBUG_LEVEL', 1);								// Debug level 1 is less noisy and 3 is the most noisy
if(! defined('MEMORY_LIMIT') )				define ('MEMORY_LIMIT', '512M');							// Set PHP memory limit, was 200; pimped to 512
...
if(! defined('FILE_CACHE_TIME_BETWEEN_CLEANS'))	define ('FILE_CACHE_TIME_BETWEEN_CLEANS', 5184000);	// How often the cache is cleaned, 5184000 = 2 monthes | 604800 = 1 week | 86400 = 1 day
...
if(! defined('FILE_CACHE_SUFFIX') ) 		define ('FILE_CACHE_SUFFIX', '.thumb');			// What to put at the end of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_PREFIX') ) 		define ('FILE_CACHE_PREFIX', 'plx');				// What to put at the beg of all files in the cache directory so we can identify them
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', '../../tmp/cache/plxthumbnailer');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
if(! defined('MAX_FILE_SIZE') )				define ('MAX_FILE_SIZE', 209715200);						// 10 Megs is 10485760. This is the max internal or external file size that we'll process. was 40485760 for long ; switching to 209715200 (200MB) 
if(! defined('CURL_TIMEOUT') )				define ('CURL_TIMEOUT', 20);							// Timeout duration for Curl. This only applies if you have Curl installed and aren't using PHP's default URL fetching mechanism.
if(! defined('WAIT_BETWEEN_FETCH_ERRORS') )	define ('WAIT_BETWEEN_FETCH_ERRORS', 3600);				// Time to wait between errors fetching remote file

//Browser caching
if(! defined('BROWSER_CACHE_MAX_AGE') ) 	define ('BROWSER_CACHE_MAX_AGE', 5184000);				// Time to cache in the browser
...
class timthumb {
	...
	public function __construct(){
		...
		if($this->isURL){
			if(ALLOW_ALL_EXTERNAL_SITES){
				...
				if(! $allowed){
					return $this->error("You may not fetch images from that site. To enable this site, you can either add it to \$ALLOWED_SITES and set ALLOW_EXTERNAL=true. Or you can set ALLOW_ALL_EXTERNAL_SITES=true, depending on your security needs.");
				}
			}
		}
	...
	protected function serveErrors(){
		...
		$html .= '</ul>';
		echo '<h1>Error has occured</h1>The following error(s) occured:<br />' . $html . '<br />';
		echo '<br />Query String : ' . htmlentities ($_SERVER['QUERY_STRING']);
		echo '<br />version : ' . VERSION . '.555</pre>';
	}
	...
	protected function processImageAndWriteToCache($localImage){
		...
		if ($zoom_crop > 0) {
          ...
          if ($width * 4 < $height) {
            # detect long single page picture , at least 4 time longer than larger, crop at middle
            $src_y = round (($height - ($height / $cmp_y * $cmp_x)) / 2);
	...
	protected function getLocalImagePath($src){
		...
		if(! $this->docRoot){
			...
			return $this->error("Could not find your website document root and the file specified doesn't exist in directory. We don't support serving files outside timthumb's directory without a document root for security reasons.");
	...
	protected function sanityFail($msg){
		return $this->error("There is a problem. Message: Please report this error $msg");
	}
