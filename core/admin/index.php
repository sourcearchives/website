	...
	<table id="articles-table" class="full-width">
		...
		if($arts) { # On a des articles
			...
			while($plxAdmin->plxRecord_arts->loop()) { # Pour chaque article
				...
				$thumb = $plxAdmin->plxRecord_arts->f('thumbnail');
        ...
        # Peppercarrot modification
				echo '<td class="wrap"><img src="../../plugins/vignette/plxthumbnailer.php?src=../../'.$thumb.'&amp;w=40&amp;h=20&amp;s=1&amp;q=92&amp;a=t"> <a href="article.php?a='.$idArt.'" title="'.L_ARTICLE_EDIT_TITLE.'">'.plxUtils::strCheck($plxAdmin->plxRecord_arts->f('title')).'</a>'.$draft.$awaiting.'&nbsp;</td>';
				echo '<td>'.$libCats.'&nbsp;</td>';
